<?php
    ...
    class NivelAguaBajoDatos {

        private $table_name = "ni_deaguabajo";

        function getUltimoRegistro($calderaId) {
            $conexion = new Conexion();
            $conn = $conexion->get_conn();

            $sql = "SELECT TOP 1 id, NivelB as valor, FORMAT(fecha, 'dd/MM/yyyy HH:mm:ss') as fechaPretty FROM ". $this->table_name ." WHERE id_caldera = ? ORDER BY id DESC";
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						...
						error_log( "message[nivel_agua_alto.php-getUltimoRegistro]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
