<?php
    ...
    class PresionBunkerDatos {

        private $table_name = "prebunker";

        function getUltimoRegistro($calderaId) {
            $conexion = new Conexion();
            $conn = $conexion->get_conn();

            $sql = "SELECT TOP 1 id, PreBunke as valor, FORMAT(fecha, 'dd/MM/yyyy HH:mm:ss') as fechaPretty FROM ". $this->table_name ." WHERE id_caldera = ? ORDER BY id DESC";
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						...
						error_log( "message[presion_bunker.php-getUltimoRegistro]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						error_log( "SQLSTATE: ".$error[ 'SQLSTATE']."<br />");
						error_log( "code: ".$error[ 'code']."<br />");
						error_log( "message[presion_bunker.php-getCantidadAlertasPorFechaPorCaldera]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
