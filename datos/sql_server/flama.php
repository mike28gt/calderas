<?php
    ...
    class FlamaDatos {
        ...
        private $table_name = "flama";

        function getUltimoRegistro($calderaId) {
            $conexion = new Conexion();
            $conn = $conexion->get_conn();

            $sql = "SELECT TOP 1 id, flama as valor, FORMAT(fecha, 'dd/MM/yyyy HH:mm:ss') as fechaPretty FROM ". $this->table_name ." WHERE id_caldera = ? ORDER BY id DESC";
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						...
						error_log( "message[flama.php-getUltimoRegistro]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						error_log( "SQLSTATE: ".$error[ 'SQLSTATE']."<br />");
						error_log( "code: ".$error[ 'code']."<br />");
						error_log( "message[flama.php-getCantidadAlertasPorFechaPorCaldera]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
