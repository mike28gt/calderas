<?php
    ...
    class PresionVaporBajaDatos {

        private $table_name = "pr_bajavapor";

        function getUltimoRegistro($calderaId) {
            $conexion = new Conexion();
            $conn = $conexion->get_conn();

            $sql = "SELECT TOP 1 id, PreBaja as valor, FORMAT(fecha, 'dd/MM/yyyy HH:mm:ss') as fechaPretty FROM ". $this->table_name ." WHERE id_caldera = ? ORDER BY id DESC";
            ...
            if ($stmt === false) {
				if (($errors = sqlsrv_errors()) != null) {
					foreach ($errors as $error) {
						error_log( "SQLSTATE: ".$error[ 'SQLSTATE']."<br />");
						error_log( "code: ".$error[ 'code']."<br />");
						error_log( "message[presion_vapor_baja.php-getCantidadAlertasPorFechaPorCaldera]: ".$error[ 'message']."<br />");
					}
				}
			}
			else {
